...
function sumaExp() {
  ...
  let tiempo = setInterval(() => {
    cantidad += 1;
    numero.textContent = "Experiencia : " + cantidad;
...
function valorNombre(event){
  ...
  boton.addEventListener("click",  localStorageGuardar(document.getElementById("nombre").value))
  boton.addEventListener("click",  alerta1)
...
function alerta1(){
  let trainer1temp=JSON.parse(localStorage.getItem("entrenador"))
Swal.fire({
  title: 'Felicidades ' +  trainer1temp.name,
  imageUrl: 'https://assetsio.reedpopcdn.com/slowpoke-pokemon.jpg?width=1200&height=900&fit=crop&quality=100&format=png&enable=upscale&auto=webp',
  imageWidth: 300,
  imageHeight: 200,
  showDenyButton: true,
  showCancelButton: false,
  confirmButtonText: 'Volver a jugar',
  denyButtonText: `Contemplar triunfo`,
}).then((result) => {
  ...
  if (result.isConfirmed) {
    ...
    Swal.fire({
      title: 'Da click en enviar al final', 
    imageUrl: 'https://mir-s3-cdn-cf.behance.net/project_modules/hd/8b960e85702783.5d8389933d7cb.png',
  imageWidth: 300,
  imageHeight: 200,
     })
  }
